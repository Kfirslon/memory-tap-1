...
import { Search, BrainCircuit, Home as HomeIcon, Sparkles, User as UserIcon, LogOut, TrendingUp } from 'lucide-react';
...
export default function Home() {
    ...
    return (
        <div className="min-h-screen bg-midnight-950 text-white font-sans pb-24 selection:bg-primary-500/30">
            {/* Background Effects */}
            <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
                <div className="absolute -top-[10%] -right-[10%] w-[60vw] h-[60vw] rounded-full bg-primary-900/20 blur-[100px] animate-pulse-slow" />
                <div className="absolute top-[20%] -left-[10%] w-[50vw] h-[50vw] rounded-full bg-accent-900/20 blur-[100px] animate-pulse-slow" style={{ animationDelay: '1s' }} />
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[80vw] h-[40vw] bg-midnight-900/50 blur-[120px]" />
            </div>

            {/* Content Container */}
            <div className="relative z-10 max-w-2xl mx-auto px-4 pt-6 min-h-screen flex flex-col">
                {/* Header */}
                <header className="mb-8 flex justify-between items-center sticky top-0 bg-midnight-950/80 backdrop-blur-xl z-20 py-4 -mx-4 px-4 border-b border-white/5">
                    {activeTab === 'memories' ? (
                        <div className="relative w-full group">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-3.5 bg-midnight-900/50 border border-white/10 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 text-white placeholder:text-slate-500 transition-all shadow-inner"
                            />
                        </div>
                    ) : (
                        <div className="flex items-center gap-3">
                            <div className="bg-gradient-to-br from-primary-500 to-accent-500 p-2.5 rounded-xl text-white shadow-glow">
                                <BrainCircuit size={24} />
                            </div>
                            <span className="font-bold text-2xl tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">Memory Tap</span>
                        </div>
                    )}
                </header>
                                    ...
                                    {(['all', 'task', 'reminder', 'idea', 'note'] as const).map((cat) => (
                                        <button
                                            ...
                                            className={`px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all border ${filterCategory === cat
                                                ...
                                                : 'bg-midnight-800/50 border-white/5 text-slate-400 hover:bg-midnight-800 hover:text-white'
                                                }`}
                                        >
                                ...
                                <div className="space-y-4 pb-32">
                                    ...
                                    {filteredMemories.length === 0 && (
                                        <div className="text-center py-20 text-slate-500">
                                            ...
                                            <p className="text-sm opacity-60">Tap the mic to start remembering</p>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
                        ...
                        {activeTab === 'profile' && (
                            <motion.div key="profile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <div className="glass-card rounded-3xl p-8 text-center mb-6 relative overflow-hidden">
                                    ...
                                    <div className="w-24 h-24 bg-gradient-to-br from-primary-500 to-accent-500 rounded-full mx-auto mb-6 flex items-center justify-center shadow-glow p-1">
                                        <div className="w-full h-full bg-midnight-900 rounded-full flex items-center justify-center">
            ...
            <AnimatePresence>
                {activeTab === 'memories' && (
                    <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="fixed bottom-28 left-0 right-0 z-40 flex justify-center pointer-events-none">
                        <div className="pointer-events-auto">
                            <RecordButton isRecording={isRecording} isProcessing={isProcessing} onStart={startRecording} onStop={stopRecording} />
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm px-4">
                <div className="glass backdrop-blur-xl p-1.5 rounded-full shadow-2xl flex items-center justify-between gap-1 border border-white/10">
                    <TabButton active={activeTab === 'memories'} onClick={() => setActiveTab('memories')} icon={<HomeIcon />} label="Home" />
...
const TabButton = ({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) => (
    ...
    >
        {active && (
            <motion.div
                layoutId="activeTab"
                className="absolute inset-0 bg-gradient-to-r from-primary-600 to-accent-600 rounded-full shadow-glow"
